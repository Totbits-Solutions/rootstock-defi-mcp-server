...
def gateway() -> Web3BlockchainGateway:
    with patch("src.infrastructure.blockchain.gateway.AsyncWeb3") as mock_web3_cls:
        mock_w3 = MagicMock()
...
class TestCall:
    ...
    async def test_call_wraps_exception_in_blockchain_query_error(self, gateway: Web3BlockchainGateway) -> None:
        mock_contract = MagicMock()
    ...
    async def test_call_with_unknown_contract_raises(self, gateway: Web3BlockchainGateway) -> None:
        with pytest.raises(BlockchainQueryError, match="Unknown contract"):
            await gateway.call("nonexistent_contract", "someFunction")


class TestIsConnected:
    async def test_is_connected_delegates(self, gateway: Web3BlockchainGateway) -> None:
        gateway._w3.is_connected = AsyncMock(return_value=True)
        assert await gateway.is_connected() is True

    async def test_is_connected_returns_false_on_error(self, gateway: Web3BlockchainGateway) -> None:
...
class TestGetBlockNumber:
    async def test_get_block_number_delegates(self, gateway: Web3BlockchainGateway) -> None:
    ...
    async def test_get_block_number_raises_on_error(self, gateway: Web3BlockchainGateway) -> None:
        gateway._w3.eth.get_block_number = AsyncMock(side_effect=Exception("connection lost"))
...
class TestContractCache:
    async def test_contract_is_cached_after_first_call(self, gateway: Web3BlockchainGateway) -> None:
        mock_contract = MagicMock()
